...
def DownloadData(Dir, Var, Startdate, Enddate, latlim, lonlim, Waitbar, cores,
    ...
    yID = np.int16(np.array([np.ceil((latlim[0] - 25.0625) * 8),
                             np.floor((latlim[1] - 25.0625) * 8)]))
    xID = np.int16(np.array([np.floor((lonlim[0] + 124.9375) * 8),
                             np.ceil((lonlim[1] + 124.9375) * 8)]))
        ...
        if not os.path.isfile(BasinDir):
            ...
            while downloaded == 0:
                try:
                    ...
                    # Define time
                    zID = int(((Date - pd.Timestamp("1979-1-1")).days) * 24) + (period - 1) - 13
            ...
            # define geo
            lonlimNLDAS = xID[0] * 0.125 - 124.9375
            latlimNLDAS = (yID[1] + 1) * 0.125 + 25.0625
...
def RetrieveData_daily(Date, args):
    ...
    for T in types:
        ...
        if not os.path.isfile(BasinDir):
            ...
            zID_start = int(((Date - pd.Timestamp("1979-1-1")).days) * 24) - 13
